...
mod tests {
    ...
    async fn execute_find_similar_supports_fuzzy_source_path() {
        let ports = FakeFindSimilarPorts {
            docs: vec![doc("journal/Duch Rewolucji.md", &[]), doc("other.md", &[])],
            chunks: HashMap::from([(
                "journal/Duch Rewolucji.md".to_string(),
                vec![StoredChunk {
                    note_path: "journal/Duch Rewolucji.md".to_string(),
                    breadcrumb: "journal/Duch Rewolucji.md > Intro".to_string(),
                    content: String::new(),
                    raw_content: String::new(),
                    vector: vec![0.1, 0.2],
                    chunk_index: 0,
